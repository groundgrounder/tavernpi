...
/** m4 运行时扩展选项：npc + story 常开；stylize 由 --stylize/--style 控制（fork 重建复用）。 */
function runtimeExtras(ctx: CliCtx): { npc: { enabled: boolean }; story: { enabled: boolean }; stylize?: StylizeRuntimeOptions } {
	return {
		npc: { enabled: true },
		story: { enabled: true },
		stylize: ctx.stylize,
...
function printTurn(report: TurnResult): void {
	...
	if (report.data.ok) {
		...
		console.log(
			`✓ 成功（attempts=${report.data.attempts}，耗时 ${report.data.durationMs}ms）events=${a.events} new_locations=${a.newLocations} new_npcs=${a.newNpcs} npc_updates=${a.npcUpdates} moves=${a.locationMoves} world_state=${a.worldState} phase_start=${a.phaseStarted} phase_end=${a.phaseEnded} time_advance=${a.timeAdvanced ? "是" : "否"}${report.data.dropped ? `，strictDrop 剔除 ${report.data.dropped.length} 项` : ""}`,
		);
	} else {
...
function parseArgs(argv: readonly string[]): CliArgs {
	...
	for (let i = 0; i < argv.length; i++) {
		...
		if (a === "--root") {
			...
			args.style = v;
			args.stylize = true; // --style 隐含 --stylize
		} else {
...
export async function main(argv: readonly string[]): Promise<void> {
	...
	console.log(`> storyDir: ${storyState.storyDir}`);
	if (args.stylize) {
		console.log(`> stylize: 开启${args.style ? `（style=${args.style}）` : "（默认文风）"}`);
	}
	...
	try {
		...
		console.log(
			`> 故事目录保留（未删）: ${runtime.storyState.storyDir}\n> 可续写: node packages/app/src/m4-cli.ts --resume ${runtime.sessionManager.getSessionFile()}${args.stylize ? " --stylize" : ""}`,
		);
	}
}
